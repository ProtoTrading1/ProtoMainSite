-- Add third and fourth product image columns; keep archive RPCs in sync.

alter table public.website_stock
  add column if not exists image_url_three text,
  add column if not exists image_url_four text;
