...
export default function ExtendedRangePage({ addToCart, cartQtyMap = {}, cartPreferenceMap = {}, specialsMap = {}, browseCategory = '', onBrowseCategoryChange }) {
  ...
  return <section className="instore" aria-labelledby="instore-title">
    ...
    })}</nav><p className="instore-rail-hint" aria-hidden="true">Swipe or scroll for more <ArrowRight size={14} /></p></>}
    <div ref={resultsRef} className="instore-results" tabIndex={-1} aria-busy={loading}>
      ...
      {loading && <div className="instore-loading" aria-hidden="true"><ProtoLogo variant="icon" size="md" className="instore-loading-logo" /><strong>Finding Instore Products…</strong><span>Checking current stock and images</span></div>}
