...
export function compareInstoreSearch(left, right, query, { newestFirst = false } = {}) {
  const terms = words(query);
  if (newestFirst) {
    return compareProductCodes(right?.sku, left?.sku)
  ...
  if (!terms.length) {
    ...
    // Codes are the source-system chronology. Within every landing or browse
    // category, show the most recently added (highest) codes first.
    return rankDifference || compareProductCodes(right?.sku, left?.sku)
      || String(left?.name || left?.title || '').localeCompare(String(right?.name || right?.title || ''));
  }
  const score = (product) => {
