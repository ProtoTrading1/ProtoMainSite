...
import { normalizeIdentifier } from '../src/lib/identifierNormalize.js';

const searchableProducts = new WeakMap();
const finishedProductQueries = ['bracelet', 'necklace', 'earring'].map((name) => ({ name }));

function preferFinishedProducts(results, query) {
  const terms = words(query);
  // A bare product noun (including a typo/plural) expresses ready-to-wear
  // intent. Leave qualified searches such as "bracelet box" to shared ranking.
  if (terms.length !== 1 || !/^[a-z]+$/i.test(String(query).trim())) return results;
  const term = terms[0];
  const intents = fuzzyFilter(finishedProductQueries, term)
    .filter(({ name }) => Math.abs(name.length - term.length) <= 1);
  if (intents.length !== 1) return results;
  const queryId = normalizeIdentifier(query);
  // Alphabetic SKUs can also look like a product word; preserve exact lookup.
  if (results.some((product) => [product.websiteSku, product.sku, product.id, product.groupPrimarySku,
    product.barcode, product.code, product.parentSku]
    .some((value) => value && normalizeIdentifier(value) === queryId))) return results;
  const finished = [];
  const other = [];
  for (const product of results) {
    (classifyInstoreProduct(product).type === 'finished-jewellery' ? finished : other).push(product);
  }
  // Stable partition: retain shared spelling, availability and relevance order
  // within each group, and keep every matched source object in the results.
  return [...finished, ...other];
}

// Share the storefront's identifier, synonym and typo ranking. Preserve source
// objects so catalogue eligibility and purchasing identity remain unchanged.
export function searchInstoreProducts(products, query) {
  if (!String(query || '').trim()) return [...products].sort((a, b) => compareInstoreSearch(a, b, ''));
  let indexed = searchableProducts.get(products);
  if (!indexed) {
    indexed = products.map((product) => ({ ...product, name: product.name || product.title,
      code: product.code || product.sku,
      description: product.description || product.originalDescription,
      categoryPath: [product.category, discoveryGroup(product)].filter(Boolean),
    }));
    searchableProducts.set(products, indexed);
  }
  const originals = new Map(indexed.map((product, index) => [product, products[index]]));
  const ranked = fuzzyFilter(indexed, query).map((product) => originals.get(product));
  const seen = new Set(ranked);
  const remaining = products.filter((product) => !seen.has(product) && matchesInstoreSearch(product, query))
    .sort((a, b) => compareInstoreSearch(a, b, query));
  return preferFinishedProducts([...ranked, ...remaining], query);
}
