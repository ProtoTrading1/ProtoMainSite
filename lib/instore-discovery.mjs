...
// Fixed shop-window choices keep the browse rail attractive and recognisable.
// Every choice still has to be present in the live eligible catalogue; a
// missing SKU naturally falls back to the regular safe candidate.
const CURATED_TILE_SKUS = {
  'Bags & wallets': '8823100003',
  'Beads & jewellery making': '8650000017',
  Beauty: '8630340011',
  'Crafts & DIY': '8696170008',
  'Hair accessories': '8820000024',
  'Home & kitchen': '8826000044',
  'More finds': '8781830011',
  'Party items': '8640000124',
  'Soft toys': '8710140009',
  'Stationery & art': '8821210002',
  'Toys & games': '8710130002',
};

const CURATED_FEATURED_SKUS = [
  '8650000017', '8618100319', '8640000124', '8710130002', '8710140009',
  '8820000024', '8821210002', '8696170008', '8630340011', '8823100003',
  '8618100345',
];

const FEATURED_FALLBACK_AVOID = ['mask', 'facial', 'pencil case', 'pendant', 'finding', 'chain', 'clasp', 'display'];
...
// The unfiltered first page is a shop window, not an import-order list.
// Searches and category browsing retain their normal exact relevance ordering.
export function featuredInstoreProducts(products) {
  const bySku = new Map((products || []).map((product) => [String(product?.sku || '').trim().toUpperCase(), product]));
  const selected = new Set();
  const featured = [];
  const add = (product) => {
    const sku = String(product?.sku || '').trim().toUpperCase();
    if (!sku || selected.has(sku) || !product) return false;
    selected.add(sku);
    featured.push(product);
    return true;
  };

  for (const sku of CURATED_FEATURED_SKUS) add(bySku.get(sku));

  const groups = new Map();
  for (const product of products || []) {
    const sku = String(product?.sku || '').trim().toUpperCase();
    const title = productText(product).toLowerCase();
    if (selected.has(sku) || FEATURED_FALLBACK_AVOID.some((term) => title.includes(term))) continue;
    const label = discoveryGroup(product);
    const entries = groups.get(label) || [];
    entries.push(product);
    groups.set(label, entries);
  }
  const groupOrder = discoveryTiles(products).map((tile) => tile.label);
  const compareFeatured = (left, right) => compareProductCodes(right?.sku, left?.sku)
    || String(left?.name || left?.title || '').localeCompare(String(right?.name || right?.title || ''));
  for (const entries of groups.values()) entries.sort(compareFeatured);
  for (let index = 0; ; index += 1) {
    let added = false;
    for (const label of groupOrder) {
      const product = groups.get(label)?.[index];
      if (product) { add(product); added = true; }
    }
    if (!added) return featured;
  }
}
