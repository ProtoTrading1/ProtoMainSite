...
export function discoveryTiles(products) {
  ...
  for (const product of products || []) {
    ...
    const candidate = {
      sku: product.sku,
      image: product.image,
      safe: isSafeTileCandidate(product, label),
    };
    ...
    if ((candidate.safe && !tile.safe)
      || (candidate.safe === tile.safe && compareProductCodes(candidate.sku, tile.sku) > 0)) {
      tile.sku = candidate.sku;
      tile.image = candidate.image;
      tile.safe = candidate.safe;
    }
    ...
    if (label === 'Bracelets' && String(product.sku || '').startsWith('86181')
      && isSafeTileCandidate(product, 'Bracelets')
