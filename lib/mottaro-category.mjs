...
const MOTARRO_DEFAULT_PATH = ['mottaro', 'mottaro-other', 'mottaro-other-general'];

/** Parse admin-persisted mottaro_path (JSON array of Mottaro node ids). */
...
/** Derive Mottaro browse path from primary DB category labels. */
function inferMotarroPathFromPrimaryLabels(labels, tree) {
...
export function inferMotarroPathFromRow(row, tree) {
  ...
  if (labels.length) {
    const fromLabels = inferMotarroPathFromPrimaryLabels(labels, tree);
...
export function enrichMotarroCategoryFields(product, row, tree, primaryPath) {
  ...
  return {
    ...
    mottaroPath: parseStoredMotarroPath(row.mottaro_path),
  };
}
