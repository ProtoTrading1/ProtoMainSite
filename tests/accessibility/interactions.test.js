...
describe('catalogue interaction accessibility', () => {
  ...
  it('dismisses the welcome banner quickly and does not restore it on this device', async () => {
    ...
    assert.match(app, /dismissWelcome\(\);\n\s+cartTriggerRef/);
    ...
    assert.match(styles, /animation: welcome-banner-brief 5\.5s ease forwards/);
    assert.match(styles, /90\.9%/);
