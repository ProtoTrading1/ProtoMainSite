...
test('the first welcome is recorded server-side and in-session as soon as it is selected', () => {
  ...
  assert.match(
    app,
    /const firstPortalLogin = isExplicitFirstPortalLogin\(customer\)[\s\S]{0,1800}rememberJourneyThisLogin\(customer\.id, loginSessionKey\)[\s\S]{0,500}if \(firstPortalLogin\)[\s\S]{0,300}markPortalWelcomeSeen/,
  );
  assert.match(
    app,
